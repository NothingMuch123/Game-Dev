...
SpriteAnimation* MeshBuilder::GenerateSpriteAnimation(const std::string & meshName, unsigned numRow, unsigned numCol)
{
	...
	for(unsigned i = 0; i < numRow; ++i)
	{
		for(unsigned j = 0; j < numCol; ++j)
		{
			...
			//v.pos.Set(0.5f, -0.5f, 0);
			v.pos.Set(1.f, 0.f, 0.f);
			v.texCoord.Set(u1 + width, v1);
			vertex_buffer_data.push_back(v);

			//v.pos.Set(0.5f, 0.5f, 0);
			v.pos.Set(1.f, 1.f, 0.f);
			v.texCoord.Set(u1 + width, v1 + height);
			vertex_buffer_data.push_back(v);

			//v.pos.Set(-0.5f, 0.5f, 0);
			v.pos.Set(0.f, 1.f, 0.f);
			v.texCoord.Set(u1, v1 + height);
			vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateMeshFromSheet(const std::string &meshName, Color color, unsigned numRow, unsigned numCol, unsigned rowFrame, unsigned colFrame, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;

	if (numRow * numCol < (rowFrame - 1) * numCol + colFrame) // Frame number wrong
	{
		return NULL;
	}
	float width = (float)1 / numCol;
	float height = (float)1 / numRow;

	// Vertex #1
	v.pos.Set(0, 0, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set((colFrame - 1) * width, (rowFrame - 1) * height);
	vertex_buffer_data.push_back(v);
	// Vertex #2
	v.pos.Set(length, 0, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(colFrame * width, (rowFrame - 1) * height);
	vertex_buffer_data.push_back(v);
	// Vertex #3
	v.pos.Set(length, length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(colFrame * width, rowFrame * height);
	vertex_buffer_data.push_back(v);
	// Vertex #4
	v.pos.Set(0, length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set((colFrame - 1) * width, rowFrame * height);
	vertex_buffer_data.push_back(v);
